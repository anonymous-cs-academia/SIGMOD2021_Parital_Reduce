...
import math
def exp_func(array, alpha):
    length = len(array)
    T = max(array)
    value = [0]*T
    for i in range(T, 0, -1):
        if i in array:
          value[i-1] = value[i-1] + math.pow(alpha, i-1)*(1.0 - alpha)
        else:
          value[0] = value[0] + math.pow(alpha, i-1)*(1.0 - alpha)
    for i in range(T, 0, -1):
        cnt = array.count(i)
        if cnt > 1:
            value[i-1] = value[i-1] / cnt
    ans = [0]*length
    base = 1.0 - math.pow(alpha, T)
    for i in range(length):
        ans[i] = value[array[i]-1]/base
    return ans
    ...
    while True:
        if(ready_queue.qsize() >= partial_reduce_num):
            ...
            for i in range(partial_reduce_num):
                ...
                time_stamp_list.append(worker_time_stamp)
            ...
            for i in range(partial_reduce_num):
                time_stamp_list[i] = max_time_stamp - time_stamp_list[i] + 1
            exp_decay_list = exp_func(time_stamp_list, 0.9)
            max_time_stamp_info = "%06d"%(max_time_stamp)
            max_time_stamp_info = max_time_stamp_info.encode('utf-8')
            for i in range(partial_reduce_num):
                exp_decay_info = "%1.6f"%(exp_decay_list[i])
                exp_decay_info = exp_decay_info.encode('utf-8')
                worker_id = group_list[i]
                comm_list[worker_id].send(send_str+max_time_stamp_info+exp_decay_info)
            #max_time_stamp_info = "%06d"%(max_time_stamp)
            #max_time_stamp_info = max_time_stamp_info.encode('utf-8')

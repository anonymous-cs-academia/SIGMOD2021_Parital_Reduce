...
def train(comm = None, epoch = 0, myrank = -1, net = None, criterion = None, optimizer = None, trainloader = None):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        loss.backward()
        # mylogger.info("after back")
        torch.cuda.synchronize()
        optimizer.step()
        param_id = 0
        for param in net.parameters():
            if param.requires_grad:
                gpu_model_buffer[param_id].copy_(param.data)
                param_id = param_id + 1
        # mylogger.info("after step")
        ...
        ''' get group '''
        group_signal = comm.recv(25)
        group_signal = group_signal.decode('utf-8')
        kgpu = int(group_signal[0])
        src_rank = int(group_signal[1])
        max_time_stamp = int(group_signal[kgpu+1:kgpu+7])
        exp_decay_info = float(group_signal[kgpu+7:kgpu+15])
        # mylogger.info("group signal"+group_signal)
        for param in net.parameters():
            if param.requires_grad:
                param.data = param.data * exp_decay_info
        ...
        if(myrank == src_rank):
            ...
            if kgpu != 1:
                for param in net.parameters():
                    if param.requires_grad:
                        # consider to change "all_reduce" to "reduce"
                        dist.all_reduce(param.data, group = allreduce_group)
                        # dist.all_reduce(param.grad, dst = src_rank, group = allreduce_group) 
        else:
            for param in net.parameters():
                if param.requires_grad:
                    # consider to change "all_reduce" to "reduce"
                    dist.all_reduce(param.data, group = allreduce_group)            
                    # dist.reduce(param.grad, dst = src_rank, group = allreduce_group)
        # mylogger.info("after allreduce")
        param_id = 0
        for param in net.parameters():
            if param.requires_grad:
                param.data = ((param.data - gpu_model_buffer[param_id] * exp_decay_info)*(kgpu-1.0)/(1.0-exp_decay_info) + gpu_model_buffer[param_id])/kgpu
                param_id = param_id + 1
